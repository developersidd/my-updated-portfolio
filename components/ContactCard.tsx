...
const ContactCard = ({ 
  ...
  return (
    <motion.div 
      ...
      className="group relative flex items-center justify-between p-5 rounded-2xl bg-[#0a0a0a] border border-white/[0.05] hover:border-green-500/20 transition-all duration-300 shadow-[0_8px_30px_rgb(0,0,0,0.12)]"
    >
      <div className="flex items-center gap-4">
        <div className="flex items-center justify-center w-11 h-11 rounded-xl bg-[#121212] border border-white/[0.05] group-hover:border-green-500/30 transition-colors">
          <Icon className="w-5 h-5 text-green-500" />
        </div>
        <div className="flex flex-col">
          <h3 className="text-[15px] font-semibold text-white/90 tracking-tight">{title}</h3>
          <p className="text-[14px] text-white/40 font-medium">{subtitle}</p>
        </div>
      </div>
